...
SELECT A.id_cliente, A.nombre, A.apellidos FROM clientes AS A LEFT JOIN pedidos AS B ON A.id_cliente = B.id_cliente WHERE B.id_cliente IS NULL;
...
FROM pedidos AS A LEFT JOIN clientes AS B ON A.id_cliente = B.id_cliente;
...
SELECT A.nombre, SUM(B.total) AS total_gastado FROM clientes AS A JOIN pedidos AS B ON A.id_cliente = B.id_cliente GROUP BY A.id_cliente, A.nombre HAVING SUM(B.total) > (SELECT AVG(total) FROM pedidos);
...
JOIN pedidos AS B ON A.id_cliente = B.id_cliente JOIN detalles_pedido AS C ON B.id_pedido = C.id_pedido
...
WHERE E2.nombre = 'fitness' AND B2.id_cliente = A.id_cliente);
